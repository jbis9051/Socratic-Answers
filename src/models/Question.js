...
class Question {
    ...
    static async getQuestions(siteid, limit = 20, offset = 0) {
        const {rows} = await conn.multiRow("SELECT id, creator_id, creator_username, title, created, last_modified, tag_string, score, answers, answered FROM question WHERE site_id = $1 ORDER BY last_modified DESC LIMIT $2 OFFSET  $3", [siteid, limit, offset]);
        return rows.map(row => {
